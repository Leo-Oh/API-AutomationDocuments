...
def compare_images(**kwargs):
    ...
    try:
        ...
        face_cascade = cv2.CascadeClassifier(os.getcwd()+"/modules/haarcascade_frontalface_default.xml")
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    cv2.imwrite(os.getcwd()+"/modules/rotation.jpg", rotated)
                    print("rotated")
                    #Unknown_image = face_recognition.load_image_file(f"{ROOT_DIR}\\rotation.jpg")
                    Unknown_image = face_recognition.load_image_file(os.getcwd()+"/modules/rotation.jpg")
                        ...
                        if not ret:
                            ...
                            if len(faces) >= 1:
                                ...
                                while True:
                                    ...
                                    cv2.imwrite(os.getcwd()+"/modules/video.jpg", rotated)
                                    #print("rotated")
                                    try:
                                        #Unknown_image = face_recognition.load_image_file(f"{ROOT_DIR}\\video.jpg")
                                        Unknown_image = face_recognition.load_image_file(os.getcwd()+"/modules/video.jpg")
                                        unknown_encoding = face_recognition.face_encodings(Unknown_image)[0]
                                        same_person = face_recognition.compare_faces([known_encoding], unknown_encoding)
